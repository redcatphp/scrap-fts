	...
	public function checkRule($rule, $value = '/', $userAgent = '*')
	{
		if (isset($this->rules[$userAgent]) && isset($this->rules[$userAgent][$rule])){
			foreach ($this->rules[$userAgent][$rule] as $robotRule){
				$escaped = strtr($robotRule, array("@" => "\@")); // change @ for \@
				return (bool) preg_match('@'.$escaped.'@', $value); // return match result
			}
		}
		return ($userAgent != '*') ? $this->checkRule($rule, $value) :
			( $rule === self::DIRECTIVE_ALLOW ? (!$this->checkRule(self::DIRECTIVE_DISALLOW, $value, $userAgent)) : false );
	}
